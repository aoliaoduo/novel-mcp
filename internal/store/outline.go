...
func validateLayeredIndexes(volumes []domain.VolumeOutline) error {
	for vi := range volumes {
		if volumes[vi].Index != vi+1 {
			return fmt.Errorf("layered outline volume index 非当前格式: position=%d index=%d", vi+1, volumes[vi].Index)
		}
		for ai := range volumes[vi].Arcs {
			if volumes[vi].Arcs[ai].Index != ai+1 {
				return fmt.Errorf("layered outline arc index 非当前格式: volume=%d position=%d index=%d", vi+1, ai+1, volumes[vi].Arcs[ai].Index)
			}
		}
	}
	return nil
