...
func TestLoadLayeredOutlineRejectsNonCurrentIndexes(t *testing.T) {
	s := NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatal(err)
	}
	if err := s.Outline.io.WriteJSON("layered_outline.json", []domain.VolumeOutline{{
		Index: 0,
		Arcs:  []domain.ArcOutline{{Index: 0, Title: "错误弧", Goal: "不应自动修复"}},
	}}); err != nil {
		t.Fatal(err)
	}
	if _, err := s.Outline.LoadLayeredOutline(); err == nil || !strings.Contains(err.Error(), "非当前格式") {
		t.Fatalf("malformed layered indexes must fail closed, got %v", err)
	}
}
