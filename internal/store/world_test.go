...
func TestTimeline_ReadIgnoresTailAndAppendRepairs(t *testing.T) {
	s := newTestStore(t)
	...
	s2 := NewStore(s.Dir())
	loaded, err := s2.World.LoadTimeline()
	if err != nil || len(loaded) != 1 || loaded[0].Event != "完整记录" {
		t.Fatalf("read must ignore uncommitted tail: got (%+v, %v)", loaded, err)
	}
	...
	if string(afterRead) != string(beforeRead) {
		t.Fatal("read path must not truncate or otherwise mutate uncommitted tail")
	}
