...
func (s *DraftStore) ExtractDialogue(characterName string, aliases []string, maxSamples, maxCompletedChapter int) ([]string, error) {
	return s.extractDialogue(characterName, aliases, maxSamples, 1, maxCompletedChapter)
}

// ExtractRecentDialogue 只在最近 lookback 章内寻找对话样本。上下文自动注入需要的是
// 当前声音而不是全书考古；显式 read_chapter(character=...) 仍使用 ExtractDialogue 全量查找。
func (s *DraftStore) ExtractRecentDialogue(characterName string, aliases []string, maxSamples, maxCompletedChapter, lookback int) ([]string, error) {
	if lookback <= 0 {
		return nil, nil
	}
	from := max(1, maxCompletedChapter-lookback+1)
	return s.extractDialogue(characterName, aliases, maxSamples, from, maxCompletedChapter)
}

func (s *DraftStore) extractDialogue(characterName string, aliases []string, maxSamples, minChapter, maxCompletedChapter int) ([]string, error) {
	if maxSamples <= 0 {
		maxSamples = 5
	}
	...
	var samples []string
	var readErrs []error
	for ch := maxCompletedChapter; ch >= minChapter && len(samples) < maxSamples; ch-- {
		text, err := s.LoadChapterText(ch)
		if err != nil {
...
// ExtractRecentStyleAnchors 从最近 lookback 章向前抽取风格锚点，避免长篇在没有合格
// 段落时每次扫描整本终稿。旧的 ExtractStyleAnchors 保留其“从开篇开始”语义。
func (s *DraftStore) ExtractRecentStyleAnchors(maxAnchors, maxCompletedChapter, lookback int) ([]string, error) {
	if maxAnchors <= 0 {
		maxAnchors = 5
	}
	if lookback <= 0 || maxCompletedChapter <= 0 {
		return nil, nil
	}
	minChapter := max(1, maxCompletedChapter-lookback+1)
	var anchors []string
	var readErrs []error
	for ch := maxCompletedChapter; ch >= minChapter && len(anchors) < maxAnchors; ch-- {
		text, err := s.LoadChapterText(ch)
		if err != nil {
			readErrs = append(readErrs, fmt.Errorf("chapter %d: %w", ch, err))
			continue
		...
		for _, para := range paragraphs {
			...
			para = strings.TrimSpace(para)
			runeCount := utf8.RuneCountInString(para)
			if runeCount < 50 || runeCount > 300 || strings.Count(para, "\u201c") > 2 {
				continue
			}
			anchors = append(anchors, para)
		}
