...
func (t *SaveFoundationTool) Schema() map[string]any {
	return schema.Object(
		...
		schema.Property("content", map[string]any{
			"description": "内容。premise 必须传 Markdown 字符串；其他类型必须直接传 JSON 数组或对象。",
		}).Required(),
...
func (t *SaveFoundationTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	content, err := normalizeFoundationContent(a.Type, a.Content)
	if err != nil {
		return nil, err
	}
	if a.Scale != "" {
...
func normalizeFoundationContent(typeName string, raw json.RawMessage) (string, error) {
	if len(raw) == 0 {
	...
	if typeName == "premise" {
		...
		if err := json.Unmarshal(raw, &text); err != nil {
			return "", fmt.Errorf("premise content 必须是 Markdown 字符串: %w", errs.ErrToolArgs)
		}
		return text, nil
	}
	var wrapped string
	if err := json.Unmarshal(raw, &wrapped); err == nil {
		return "", fmt.Errorf("%s content 必须直接传 JSON 数组或对象，不能再包成 JSON 字符串: %w", typeName, errs.ErrToolArgs)
	}

	if !json.Valid(raw) {
		return "", fmt.Errorf("invalid content: expected valid JSON array or object: %w", errs.ErrToolArgs)
	}
