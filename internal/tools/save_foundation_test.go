...
func TestSaveFoundationOutlineClearsLayeredStateWhenDowngrading(t *testing.T) {
	...
	layeredArgs, err := json.Marshal(map[string]any{
		...
		"content": json.RawMessage(`[{"index":1,"title":"第一卷","theme":"主题","arcs":[{"index":1,"title":"第一弧","goal":"目标","chapters":[{"chapter":1,"title":"第一章","core_event":"开局","hook":"继续"}]}]}]`),
		"scale":   "long",
	})
	if err != nil {
	...
	outlineArgs, err := json.Marshal(map[string]any{
		"type":    "outline",
		"content": json.RawMessage(`[{"chapter":1,"title":"第一章","core_event":"改为中篇","hook":"继续"}]`),
		"scale":   "mid",
	})
	if err != nil {
...
func TestSaveFoundationRejectsStringWrappedJSON(t *testing.T) {
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatal(err)
	}
	...
	args, _ := json.Marshal(map[string]any{
		"type":    "outline",
		"content": `[{"chapter":1,"title":"第一章","core_event":"开局"}]`,
	})
	if _, err := NewSaveFoundationTool(s).Execute(context.Background(), args); err == nil || !strings.Contains(err.Error(), "不能再包成 JSON 字符串") {
		t.Fatalf("string-wrapped JSON must be rejected, got %v", err)
	}
}
