...
import (
	...
	"novel-mcp/internal/llmcontract"
...
func TestAuditFoundationCheckpointFailureDoesNotEnterWritingAndCanRetry(t *testing.T) {
	s := completeShortFoundation(t)
	fingerprint, err := s.FoundationFingerprint()
	if err != nil {
		t.Fatal(err)
	...
	args, _ := json.Marshal(map[string]any{
		"fingerprint": fingerprint, "ready": true, "summary": "基础设定一致", "issues": []any{},
	})
	...
	tool := NewAuditFoundationTool(s)
	if _, err := tool.Execute(context.Background(), args); err == nil || !strings.Contains(err.Error(), "checkpoint foundation audit") {
		t.Fatalf("expected checkpoint failure, got %v", err)
	}
	if p, _ := s.Progress.Load(); p.Phase == domain.PhaseWriting {
		t.Fatal("checkpoint 失败时不得提前进入 writing")
	}
	audit, err := s.Outline.LoadFoundationAudit()
	if err != nil || audit == nil || !audit.Ready {
		t.Fatalf("审查工件应已持久化，便于安全重试: audit=%+v err=%v", audit, err)
	}
	if err := os.Remove(checkpointPath); err != nil {
		t.Fatal(err)
	}
	if _, err := tool.Execute(context.Background(), args); err != nil {
		t.Fatalf("恢复 checkpoint 路径后应可安全重试: %v", err)
	}
	if p, _ := s.Progress.Load(); p.Phase != domain.PhaseWriting {
		t.Fatalf("重试成功后应进入 writing，got %s", p.Phase)
	}
}
