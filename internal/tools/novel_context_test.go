...
func TestContextToolInjectsRewriteBriefForPendingRewriteChapter(t *testing.T) {
	...
	if err := s.World.SaveReview(domain.ReviewEntry{
		...
		Issues: []domain.ConsistencyIssue{
			{Type: "pacing", Severity: "error", Description: "前 2000 字无推进", Chapters: []int{2}, RequiresChange: true},
		},
	...
	reviews, _ := brief["reviews"].([]any)
	if len(reviews) != 1 {
		t.Fatalf("expected one review source, got %v", brief["reviews"])
	}
	review, _ := reviews[0].(map[string]any)
	if got, _ := review["summary"].(string); !strings.Contains(got, "铺垫过长") {
		t.Fatalf("expected review summary from chapter review, got %v", review["summary"])
	}
	if issues, _ := review["issues"].([]any); len(issues) == 0 {
		t.Fatalf("expected review issues in rewrite_brief.reviews, got %v", review["issues"])
	}
	if misses, _ := review["contract_misses"].([]any); len(misses) == 0 {
		t.Fatalf("expected contract misses in rewrite_brief.reviews, got %v", review["contract_misses"])
	}
}
...
func TestContextToolLoadsArcReviewAffectingEarlierChapter(t *testing.T) {
	...
	reviews, _ := brief["reviews"].([]any)
	if brief == nil || len(reviews) == 0 || !strings.Contains(fmt.Sprint(reviews[0].(map[string]any)["summary"]), "第二弧") {
...
func TestContextToolComputesRuleViolationsFromSystemDefaults(t *testing.T) {
	dir := t.TempDir()
	st := store.NewStore(dir)
	if err := st.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	...
	if _, err := st.ChapterRecords.Accept(2, domain.ChapterOriginGenerated, "某种程度上，他回头了。", domain.ChapterFacts{
	...
	if !ok || len(vs) == 0 {
		t.Fatalf("rule_violations 必须反映服务端机械基线, got %v", result["rule_violations"])
	}
}
