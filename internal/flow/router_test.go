...
func TestRoute_PlanningFillDispatchesSamePlanner(t *testing.T) {
	...
	compassMissing := base
	compassMissing.PlanningTier = domain.PlanningTierLong
	compassMissing.FoundationMissing = []string{"compass"}
	gotCompass := Route(compassMissing)
	if gotCompass == nil || len(gotCompass.Actions) != 1 || gotCompass.Actions[0].Tool != "save_foundation" {
		t.Fatalf("compass 缺失时应给结构化 save_foundation 动作,got %+v", gotCompass)
	}
	if gotCompass.Actions[0].Arguments["type"] != "update_compass" {
		t.Fatalf("FoundationMissing 的 compass 必须映射为 type=update_compass: %#v", gotCompass.Actions[0])
	}

	longOutlineMissing := base
	longOutlineMissing.PlanningTier = domain.PlanningTierLong
	longOutlineMissing.FoundationMissing = []string{"outline"}
	gotLongOutline := Route(longOutlineMissing)
	if gotLongOutline == nil || gotLongOutline.Actions[0].Arguments["type"] != "layered_outline" || gotLongOutline.Actions[0].Arguments["scale"] != "long" {
		t.Fatalf("long 的 outline 缺项必须映射为 layered_outline: %#v", gotLongOutline)
	}

	shortOutlineMissing := base
	shortOutlineMissing.PlanningTier = domain.PlanningTierShort
	shortOutlineMissing.FoundationMissing = []string{"outline"}
	gotShortOutline := Route(shortOutlineMissing)
	if gotShortOutline == nil || gotShortOutline.Actions[0].Arguments["type"] != "outline" || gotShortOutline.Actions[0].Arguments["scale"] != "short" {
		t.Fatalf("short 的 outline 缺项必须保持扁平 outline: %#v", gotShortOutline)
	}
...
func TestRoute_ActionsEncodeRequiredAndChoiceSemantics(t *testing.T) {
	p := &domain.Progress{
		Phase: domain.PhaseWriting, Flow: domain.FlowWriting,
		CompletedChapters: []int{1}, TotalChapters: 5,
	}
	feedback := Route(State{
		Progress: p, LastCompleted: 1, PlanningTier: domain.PlanningTierLong,
		ImmediateFeedbackCount: 1,
	})
	if feedback == nil || len(feedback.Actions) != 3 {
		t.Fatalf("feedback route actions=%#v", feedback)
	}
	if feedback.Actions[0].Tool != "novel_context" || feedback.Actions[0].Mode != RouteActionRequired {
		t.Fatalf("feedback 第一动作必须先读上下文: %#v", feedback.Actions[0])
	}
	for _, a := range feedback.Actions[1:] {
		if a.Mode != RouteActionChoice || a.ChoiceGroup != "feedback_resolution" {
			t.Fatalf("feedback 处理动作必须是同组互斥选择: %#v", feedback.Actions)
		}
	}

	exhausted := &domain.Progress{
		Phase: domain.PhaseWriting, Flow: domain.FlowWriting,
		CompletedChapters: []int{1}, TotalChapters: 1,
	}
	decision := Route(State{Progress: exhausted, LastCompleted: 1, PlanningTier: domain.PlanningTierShort})
	if decision == nil || len(decision.Actions) != 2 {
		t.Fatalf("outline exhausted actions=%#v", decision)
	}
	if decision.Actions[0].Tool != "save_foundation" || decision.Actions[1].Tool != "revise_outline" {
		t.Fatalf("outline exhausted 选择项错误: %#v", decision.Actions)
	}
	for _, a := range decision.Actions {
		if a.Mode != RouteActionChoice || a.ChoiceGroup != "outline_exhausted" {
			t.Fatalf("outline exhausted 必须显式互斥: %#v", decision.Actions)
		}
	}
}
