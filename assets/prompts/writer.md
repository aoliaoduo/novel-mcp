...
`working_memory.user_rules.structured` 是服务端确定性机械规则（forbidden_chars、forbidden_phrases、fatigue_words）。提交时会按这些规则检查正文；它不是额外的用户需求来源。

## 字数

章节长短由叙事节奏决定：按题材常规与本章剧情承载量自然收束，不为凑字灌水，也不为压缩砍掉必要铺垫。
