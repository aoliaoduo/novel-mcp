...
`novel_context` 返回的 `working_memory.user_rules.structured` 是服务端机械规则基线（forbidden_chars / forbidden_phrases / fatigue_words / genre）。

`novel_context(chapter=N)` 会按接纳正文即时计算机械检查结果，并通过顶层 `rule_violations` 数组提供（无违规时字段缺省）。机械违规优先映射进现有基础维度，不要为每条规则机械制造新维度：
...
章节长短没有机械规则：篇幅是否配得上剧情承载量，属于 pacing 维度的语义判断。
