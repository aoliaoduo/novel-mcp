...
2. **Origin 策略**：浏览器跨源请求带 `Origin`。`allow_origins` 支持精确源，也支持 `"*"`；
   当前 local/public 默认使用 `"*"`，因此任意网页 Origin 都能完成 CORS 预检。Origin 默认不再作为
...
* **默认允许任意网页 Origin**：这是为了让不同网页 MCP Host 无需逐个维护域名。恶意网页仍需要同时拿到
  秘密 route 与 Bearer 才能调用工具；因此不要把完整 MCP 配置粘贴到不可信网站，也不要公开截图凭据。
